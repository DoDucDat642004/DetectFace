...
from gender_race_age.load_gra_model import load_model
from gender_race_age.predict import predict_from_image, predict_from_url

# Cấu hình
YOLO_MODEL_PATH = "./yolo/model/yolo11n_openvino_model/"
GRA_MODEL_PATH = "./gender_race_age/model/best_model.pt"

def predict_single_url_with_crop(url, gra_model, yolo_model):
    try:
        # Gửi request
...
if __name__ == "__main__":
    ...
    gra_model = load_model(GRA_MODEL_PATH)
    gra_model.to(device)
    ...
    if gra_model:
        predict_single_url_with_crop(test_url, gra_model, yolo_model)

# Run : python -m gender_race_age.main